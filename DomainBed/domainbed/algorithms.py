...
class IDM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        assert backpack is not None, "Install backpack with: 'pip install backpack-for-pytorch==1.3.0'"
        super(IDM, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.num_domains = num_domains
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        feat_penalty = self.compute_feature_penalty(all_z, len_minibatches)
        grad_penalty = self.compute_gradient_penalty(all_logits, all_y, len_minibatches)
        all_nll = F.cross_entropy(all_logits, all_y)

        feat_penalty_weight = self.hparams["feat_lambda"]
        grad_penalty_weight = 0
        if self.update_count >= self.hparams["grad_penalty_anneal_iters"]:
            grad_penalty_weight = self.hparams["grad_lambda"]
            if self.update_count == self.hparams["grad_penalty_anneal_iters"] != 0:
                # Reset Adam as in IRM or V-REx, because it may not like the sharp jump in
                # gradient magnitudes that happens at this step.
                self._init_optimizer()
        self.update_count += 1

        objective = all_nll + grad_penalty_weight * grad_penalty + feat_penalty_weight * feat_penalty
        self.optimizer.zero_grad()
        objective.backward()
        self.optimizer.step()

        return {'loss': objective.item(), 'nll': all_nll.item(), 'feat_penalty': feat_penalty.item(), 'grad_penalty': grad_penalty.item()}

    def compute_feature_penalty(self, all_z, len_minibatches):
        all_idx = 0
        feature_per_domain = [all_z for _ in range(self.num_domains)]
        for domain_id, bsize in enumerate(len_minibatches):
            env_feature = all_z[all_idx:all_idx + bsize]
            all_idx += bsize
            feature_per_domain[domain_id] = env_feature.sort(dim=0).values

        feature_average = torch.stack(
            [
                feature_per_domain[domain_id]
                for domain_id in range(self.num_domains)
            ],
            dim=0
        ).mean(dim=0)
        ...
        penalty = 0
        for domain_id in range(self.num_domains):
            penalty += (feature_per_domain[domain_id] - feature_average).pow(2).mean()
        return penalty / self.num_domains

    def compute_gradient_penalty(self, all_logits, all_y, len_minibatches):
        self.optimizer.zero_grad()
        all_loss = self.bce_extended(all_logits, all_y).sum()
        with backpack(BatchGrad()):
            all_loss.backward(inputs=list(self.classifier.parameters()), retain_graph=True, create_graph=True)
        ...
        # grads var per domain
        gradient_per_domain = [{} for _ in range(self.num_domains)]
        for name, _grads in all_grads.items():
            all_idx = 0
            for domain_id, bsize in enumerate(len_minibatches):
                env_grads = _grads[all_idx:all_idx + bsize]
                all_idx += bsize
                gradient_per_domain[domain_id][name] = env_grads.sort(dim=0).values

        # moving average
        for domain_id in range(self.num_domains):
            gradient_per_domain[domain_id] = self.ema_gradient[domain_id].update(
                gradient_per_domain[domain_id]
            )

        gradient_average = OrderedDict([
            (
                name,
                torch.stack(
                    [
                        gradient_per_domain[domain_id][name]
                        for domain_id in range(self.num_domains)
                    ],
                    dim=0
                ).mean(dim=0)
            )
            for name in gradient_per_domain[0].keys()
        ])

        penalty = 0
        for domain_id in range(self.num_domains):
            penalty += l2_between_dicts(gradient_per_domain[domain_id], gradient_average)
        return penalty / self.num_domains
